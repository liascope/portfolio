...
export default function Button ({link, bright = false, children, target="_self", textSize='sm:my-6 sm:px-6 py-2 my-3 px-3' }){
    ...
    if (bright) return <Link href={link} target={target} className={`bg-white/40 text-teal-800/80 text-sm sm:text-base uppercase font-bold sm:tracking-widest hover:text-teal-600/80 rounded-lg hover:rounded-sm hover:bg-white/80 transition ${textSize}`}>
            {children}
          </Link>
